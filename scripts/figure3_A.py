...
bexy_file = '02_hmp/bexy_output/hmp_bexy_output.txt'
rxry_file = '02_hmp/hmp_rxry_output.txt'
...
plt.savefig('./figures/fig3A_performance_matrices.eps', dpi=300, bbox_inches='tight', transparent=True, format='eps')
plt.show()
...
accuracy_data.to_csv(os.path.join(save_path, 'figure3A_accuracy_data.txt'), sep='\t', index=False)
...
for _, row in accuracy_data.iterrows():
    ...
    for method in methods:
        ...
        # Save each chart
        filename = f"{save_path}fig3A_{host_site}_{method}_accuracy_pie_chart.eps"
