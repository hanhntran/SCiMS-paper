...
##########################################################################################################
# Figure 3A: Bar chart of precision, recall, and F1 score for each method
...
plt.savefig('./figures/fig3B_performance_matrices.eps', dpi=300, bbox_inches='tight', transparent=True, format='eps')
plt.show()

##########################################################################################################
# Figure 3B: Pie chart of accuracy for SCiMS - pie chart is scaled by sample size
...
def calculate_accuracy_by_method(df, methods, host_sex_col):
    results = []
    for site, group in df.groupby('LibrarySource'):
        site_data = {'LibrarySource': site}
...
sample_counts = scims.groupby('LibrarySource')['Sample'].count().reset_index()
sample_counts.columns = ['LibrarySource', 'Sample_size']
accuracy_data = accuracy_data.merge(sample_counts, on='LibrarySource')

# Save accuracy data to a file
accuracy_data.to_csv(os.path.join(save_path, 'figure3B_accuracy_data.txt'), sep='\t', index=False)
...
for _, row in accuracy_data.iterrows():
    ...
    for method in methods:
        ...
        # Add the uncertainty rate at the bottom center of the pie chart
        #plt.text(
        #    0, -1.7,  # Position the text below the chart
        #    f'Uncertainty: {uncertainty_rate:.1%}', 
        #    ha='center', fontsize=50, color='gray'
        #)
        ...
        # Save each chart
        filename = f"{save_path}fig3B_{LibrarySource}_{method}_accuracy_pie_chart.eps"
